...
if (versionCompare(App.game.update.version, lastCompatibleVersion) === 1) {
  Notifier.notify({
    title:
      "WARNING: Pokéclicker version is newer than this version of Autopoke",
...
for (let i = 0, len = modules.length; i < len; i++) {
  ...
  }.js?v=${AutopokeVersion}`;
  checkReady[modules[i]] = setInterval(
    (function (i, x) {
      return function () {
        ...
        console.log(
          "Error loading " + modules[i] + ", retries: (" + x + "/10)"
        );
        if (x >= 10) {
